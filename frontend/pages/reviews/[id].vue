          ...
          <div class="md:flex md:items-start md:justify-between">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-2 flex flex-col sm:flex-row sm:flex-wrap sm:space-x-6">
                ...
                <div class="mt-2 flex items-center text-sm text-gray-500">
                  <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3a4 4 0 118 0v4m-4 6a3 3 0 100-6 3 3 0 000 6zm-7 10V3a4 4 0 118 0v14a2 2 0 01-2 2H5a2 2 0 01-2-2z"></path>
                  </svg>
          ...
          <div class="px-6 py-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              ...
              <div>
                ...
                <p class="text-gray-900">{{ formatDateTime(review.created_at) }}</p>
              </div>
              <div v-if="review.updated_at !== review.created_at">
                <label class="block font-medium text-gray-700">最終更新</label>
                <p class="text-gray-900">{{ formatDateTime(review.updated_at) }}</p>
              </div>
              <div>
...
// メタデータ設定
useHead(() => ({
  title: review.value ? `${review.value.shop?.name} のレビュー - マジキチメシ` : 'レビュー詳細 - マジキチメシ',
  meta: [
    { name: 'description', content: review.value ? `${review.value.shop?.name}のレビュー詳細` : 'レビュー詳細ページ' }
  ]
}))
</script>