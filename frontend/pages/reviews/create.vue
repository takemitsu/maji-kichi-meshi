            ...
            <form @submit.prevent="submitReview" class="space-y-6">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div v-if="selectedShop" class="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                        ...
                        <button @click="clearSelectedShop" type="button" class="text-gray-400 hover:text-gray-600">
...
const canSubmit = computed(() => {
    const result = form.value.shop_id && form.value.rating > 0 && form.value.repeat_intention && form.value.visited_at
    ...
    try {
        ...
        if (uploadedImages.value.length > 0) {
            try {
                ...
                // 詳細エラー情報をログ出力
                if (imageErr && typeof imageErr === 'object' && 'status' in imageErr && 'data' in imageErr) {
                    const errorObj = imageErr as { status: number; data: { messages?: string; error?: string } }
                    ...
                    // エラーレスポンスの詳細内容を表示
                    if (errorObj.data && errorObj.data.messages) {
                        console.error('Validation errors:', errorObj.data.messages)
                    }
                    if (errorObj.data && errorObj.data.error) {
                        console.error('Error message:', errorObj.data.error)
                    }
                }
                ...
                // 画像アップロードに失敗してもレビューは作成されているので詳細ページに遷移
                setTimeout(async () => {
...
const setError = (errorInfo: {
    ...
    // エラーメッセージを表示した後、ページ上部にスムーズスクロール
    nextTick(() => {
