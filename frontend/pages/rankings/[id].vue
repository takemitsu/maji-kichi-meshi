        ...
        <div class="mt-8 bg-gray-50 rounded-lg p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            ...
            <div>
              ...
              <p class="text-gray-900">{{ formatDateTime(ranking.created_at) }}</p>
            </div>
            <div v-if="ranking.updated_at !== ranking.created_at">
              <label class="block font-medium text-gray-700">最終更新</label>
              <p class="text-gray-900">{{ formatDateTime(ranking.updated_at) }}</p>
            </div>
            <div>
              <label class="block font-medium text-gray-700">公開状態</label>
              <p class="text-gray-900">{{ ranking.is_public ? '公開' : '非公開' }}</p>
            </div>
          </div>
        </div>
...
useHead(() => ({
  ...
  meta: [
    { name: 'description', content: ranking.value ? `${ranking.value.title}の詳細ランキング` : 'ランキング詳細ページ' }
  ]
}))
</script>